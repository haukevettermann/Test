FUNCTION zmhp_kapatool_get_tab_wconfig.
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     VALUE(IV_PERNR) TYPE  PERNR_D OPTIONAL
*"  EXPORTING
*"     VALUE(ES_ZMHP_KAPATOOL_WC) TYPE  ZMHP_KAPATOOL_WC
...
*"----------------------------------------------------------------------
  ...
  IF iv_pernr IS INITIAL.
    ...
    IF lv_pernr IS NOT INITIAL.
      ...
      SELECT SINGLE * FROM zmhp_kapatool_wc
            INTO CORRESPONDING FIELDS OF es_zmhp_kapatool_wc
  ...
  IF es_zmhp_kapatool_wc IS NOT INITIAL.
    ev_success = 'X'.
  ENDIF.
